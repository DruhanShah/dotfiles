...
DEFAULT_BG = as_rgb(color_as_int(options.tab_bar_background))
...
def single_tab_components(idx: int, tab: Tab, is_active: bool) -> List[Component]:
    fg = ACTIVE_TAB_FG if is_active else INACTIVE_TAB_FG
    fg_alt = DEFAULT_BG
    bg = ACTIVE_TAB_BG if is_active else INACTIVE_TAB_BG
    return (
        [Component(f"", ComponentOptions(fg_alt, bg, True))]
        + [Component(f" {idx+1} ", ComponentOptions(fg, bg, True))]
        + [Component(f"", ComponentOptions(fg_alt, bg, True))]
    )
...
def all_tab_components(
    ...
    for i, tab in enumerate(get_boss().all_tabs):
        next_tab_components = single_tab_components(i, tab, tab.id == active_tab_id)
...
def parse_statusline_str(statusline_str) -> List[Component]:
    ...
    stl_items.pop(0)
    parsed_statusline = []
    for i, item in enumerate(stl_items):
        ...
        for item_format in format.split(","):
            if item_format.startswith("fg=#") and item_format != "fg=default":
                fg = as_rgb(int(item_format[4:10], 16))
            elif item_format.startswith("bg=#") and item_format != "bg=default":
        ...
        if i == len(stl_items) - 1:
            with open("/tmp/kitty.log", "a") as f:
                print(f"Final length: {len(parsed_statusline)}", file=f)
