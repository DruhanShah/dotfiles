...
.hx_IssueLabel {
	...
	--border-fadeout: max(0, min(calc((var(--perceived-lightness) - var(--border-threshold)) * 100), 1));
...
.topic-tag {
	...
	--color-accent-emphasis: var(--blue);
	--color-accent-muted: fadeout(var(--blue), 60%);
	--color-accent-subtle: fadeout(var(--blue), 85%);
}
...
:where(#repo-visibility-proceed-button-public, #repo-visibility-proceed-button-private):not([data-next-stage]) {
	color: var(--color-btn-danger-text);
	background-color: var(--color-btn-danger-bg);
}
...
/* Keyboard hint */
.bdgUpN,
.hZMmEi .TextInput-icon, .hZMmEi .TextInput-action {
	color: var(--text);
	background-color: var(--mantle);
...
.hLwWNO .PRIVATE_TreeView-item[aria-current="true"] > .PRIVATE_TreeView-item-container {
	background-color: var(--surface0);
	&::after {
		background-color: var(--accent-color);
	}
...
.joxhcl, .hXyJIp, .fMkBhb > .segmentedControl-content {
	background-color: var(--mantle);
}
...
.jykkjb {
	background-color: var(--base);
	box-shadow: var(--crust) 0px -2px 0px inset, var(--crust) 0px 1px 0px inset;
}
