...
This configuration started off heavily based off of [[https://github.com/rougier][Nicolas Rougier]]'s Emacs packages and ideas. I still have a dedicated =nano/= directory for configurations more or less entirely mooched off his excellent packages. It has now grown into a monstrosity beyond attribution (shameless self praise), having incorporated ideas and packages by other giants in the Emacs world such as Karthink, Prot, and Xenodium, and will continue to grow as I understand Emacs and my own working style better. The hope is that by the end it will have condensed into a semblance of order. This file is somewhat structured after all!
...
While I do greatly enjoy Rougier's =nano-theme= and Prot's =modus-= and =ef-themes=, I would like something more tailored to my taste while still attempting to retain ideas from their themes. I'm currently choosing Steph Ango's =flexoki= theme with some modifications.
...
I'm really picking from all the greats here, and Xenodium (Álvaro) has a fantastic-looking package called Ready-Player which I'll be using to play my music locally.
...
This plugin is, as the kids say these days: a straight banger. It's technically using the Atomic Chrome extension, but the browser plugin I'm using is GhostText and they're compatible from v2.0.0 onwards.
