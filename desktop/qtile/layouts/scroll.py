...
class Column(base._ClientList):
    ...
    def maximize(self):
        """
        Temporarily set the column width to the entire screen, while storing
        the original width.
        """
...
class Scrolling(base.Layout):
    ...
    defaults = [
        ...
        ("snap", True, "Snap scrolling to view complete windows"),
    ]
    ...
    def focus(self, client):
        ...
        TODO: Update the viewx property to ensure the client is just visible.
        """
        x_coord = 0
        for i, c in enumerate(self.columns):
            if client in c:
                if self.snap and x_coord + c.width > self.viewx + 100:
                    self.viewx = x_coord + c.width - 100
                elif self.snap and x_coord < self.viewx:
                    self.viewx = x_coord
                c.focus(client)
                self.current = i
                break
    ...
    def remove_column(self, col):
        ...
        if idx <= self.current:
            ...
            if self.current == -1 and len(self.columns) > 0:
                self.current = 0
    ...
    def maximize(self):
        """
        Maximize the current column, i.e. set its width to the entire screen.
        """
        self.cc.maximize()
        self.group.layout_all()
    ...
    def reset_scroll(self):
        """
        Reset the scroll to the first column.
        """
        self.viewx = 0
        self.group.focus(self.cc.cw)
