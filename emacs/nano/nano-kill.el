...
(defun nano-kill-emacs ()
  ...
  (if (> (length (frame-list)) 1)
    ...
    (if (files--buffers-needing-to-be-saved nil)
        (let ((answer
               (nano-read-yes-or-no
                (propertize "WARNING" 'face '(:inherit error :inverse-video t))
                (propertize (format "Some buffers are modified. What do you want to do?")
                            'face 'default)
                (list (propertize "Cancel" 'face 'default)
                      (concat (propertize "Save & " 'face 'default)
  ...
  (if (and (buffer-modified-p)
    ...
    (let ((answer
           (nano-read-yes-or-no
            ...
            (list (propertize "NO" 'face 'default)
                  (propertize "YES" 'face 'error)))))
