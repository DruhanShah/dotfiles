...
(defun nano-agenda-color-luminance (color)
  "Calculate the relative luminance of COLOR (e.g. \"#ffaa00\", \"blue\").
...
(defun nano-agenda--entry-conflict (entry-1 entry-2)
  "Check if date ranges ENTRY-1 and ENTRY-2 overlap."
...
(defun nano-agenda-holidays (org-date &optional force-update)
  "Return holiday for given ORG-DATE as (month day year).
Force update if FORCE-UPDATE is t."
...
(defun nano-agenda-anniversaries (org-date &optional force-update)
  "Return anniversaries for given ORG-DATE as (month day year).
Force update if FORCE-UPDATE is t."
...
(defun nano-agenda-entries (org-date &optional force-update)
  "Return entries for given ORG-DATE as (month day year).
Force update if FORCE-UPDATE is t.
...
Force updates if FORCE-UPDATE is non-nil.  Occupancies are cached for efficiency."
