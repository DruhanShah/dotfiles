...
There's also some keybinds that I don't think make sense but I use very regularly.
...
This sets up three major plugins: Magit (all hail), Copilot, and GPTel.
...
Here, I set up the AI-based stuff like copilot (for completions) and GPTel (for other LLM and agent capabilities). Similar issues to transient ensure if I remove the ~track-changes~ installation.
...
For now, I'm just using a function to auto-tangle. I'm considering changing it to simply ~org-babel-tangle~ an org file from the init.el
