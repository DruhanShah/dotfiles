...
While I do greatly enjoy Rougier's ~nano-theme~ and Prot's ~modus-~ and ~ef-themes~, I would like something more tailored to my taste while still attempting to retain ideas from their themes. I'm going to be using Prot's ~modus-themes~ package to create a custom theme using the Flexoki palette. I used to use the ~doom-themes~ package for this, but since I made the decision to nuke dark themes from my system, Modus themes are back on the menu.
...
I used to be a big fan of Eshell and Eat because of the ability to use snippets there, but emulation proved to be a PITA.
I tried using a package that used the ~libghostty~ to emulate a terminal, and it was much nicer, but the problem of having multiple open terminal buffers was more of a pain than I thought. Also there were some emulation issues too. I suppose I'll stick to Ghostty for now until I find something more my style, or learn to using something optimally.
...
The agenda-specific stuff I'll write in another block since they're so significant in my mind. This basically replaces the fancy TODO keywords from ~org-modern~. I can make them a little nicer, but I'm lazy rn.
...
The concept of Org timestamp overlays blew my mind. Apparently the ~org-timestamp-custom-formats~ variable can just do the thing that I needed ~org-modern~ for! Now I can just set this and then set some funky faces to make it look pretty and I'm done!
...
The ~org-modern~ package by =minad= (Daniel Mendler) has been removed, since I've found more natural-feeling native ways of doing the things that I used the package for. That's not to say it's a bad package, though! I just use it for some things that I can do without.
