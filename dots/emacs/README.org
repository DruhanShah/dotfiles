...
The plugin loading as described earlier.
#+begin_src emacs-lisp
  (elpaca-wait)
#+end_src
...
Then, I yoink some code from Karthink's website that makes ~query-replace-regexp~ use ~reb~.

#+begin_src emacs-lisp
  (defvar drs/reb-pos nil
    "Store point and region bounds before calling `re-builder'.")

  (advice-add 're-builder
  	    :before
  	    (defun drs/re-builder-save-state (&rest _)
  	      "Save into `drs/reb-pos' the point and region positions before calling `re-builder'."
  	      (setq drs/reb-pos
  		    (cons (point)
  ...
  surrounded by word boundaries."
    ...
    (let* ((re (reb-target-value 'reb-regexp))
  	 (replacement (query-replace-read-to
  		       ...
  		       t))
  	 (pnt (car drs/reb-pos))
  	 (beg (cadr drs/reb-pos))
  	 (end (caddr drs/reb-pos)))
      (with-selected-window reb-target-window
        (goto-char pnt)
        (setq drs/reb-pos nil)
        (reb-quit)
        (query-replace-regexp re replacement delimited beg end))))
#+end_src

Now I believe I'm ready to make this a proper keybind-heaven.
...
#+begin_src emacs-lisp
  ...
  (use-package cdlatex
    ...
    (add-to-list 'cdlatex-math-symbol-alist '(?| "\\mapsto"  "\\longmapsto"  "\\:\\middle\\mid\\:"))
    (add-to-list 'cdlatex-math-symbol-alist '(?{ "\\subset"  "\\subseteq"))
    (add-to-list 'cdlatex-math-symbol-alist '(?} "\\superset"  "\\superseteq"))
...
Now that everything else's done, I can setup all the packages. This is because I need a couple of packages (~nerd-icons~) for what comes next.
#+begin_src emacs-lisp
  (elpaca-wait)
#+end_src
...
#+begin_src emacs-lisp
  ...
  (defun drs-mode-line-format-nothing ()
    "Modeline format to make a simple line to separate the echo area."
    `(""))
#+end_src
