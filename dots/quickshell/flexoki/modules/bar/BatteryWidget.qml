...
Rectangle {
	...
	ColumnLayout {
		...
		ColumnLayout {
			...
			Text {
			    text: batteryPopup.deviceLabel(modelData)
			    color: Theme.paper
			    font.pixelSize: 13
			    Layout.fillWidth: true
			...
			text: {
				...
				return batteryPopup.formatTime(modelData.timeToFull) + " until full";
			    if (modelData.timeToEmpty > 0)
				return batteryPopup.formatTime(modelData.timeToEmpty) + " remaining";
			    return "";
			}
			color: Theme.paper
			opacity: 0.55
